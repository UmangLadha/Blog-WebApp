...
const NewBlogs = () => {
  ...
  const handleFile = (e) => {
	...
	setBlogContent({...blogContent, blogImg: arrOfImg});
  };

  const handlePublish = (e) => {
    e.preventDefault();
    console.log("Publishing the blog", blogContent);
  };
        ...
        <div className="border shadow-lg p-5 rounded-lg">
          <form className="flex flex-col justify-between mx-auto items-start gap-2 w-4/5 md:w-3/4">
            ...
            <button
              ...
              disabled={btnActive}
              className="bg-green-600 font-semibold text-lg text-white py-2 px-4 rounded-lg w-1/3 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Publish
            </button>
          </form>
